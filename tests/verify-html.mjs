...
const checks = [
  ...
  ['semantic clusters include all launch groups', /ветровка купить/.test(html) && /значок купить/.test(html) && /шопер купить/.test(html) && /детская игра купить/.test(html) && /подарочный сертификат/.test(html)],
  ...
  ['development handoff is framed as post-approval constraints', /id="implementation-constraints"/.test(html) && /Что передать в разработку после согласования/.test(html) && !/id="dev-brief"/.test(html)],
